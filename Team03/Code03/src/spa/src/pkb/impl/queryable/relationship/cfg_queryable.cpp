...
std::unordered_set<std::string> CfgQueryable::QueryNext(int statement_number,
                                                        EntityType type) const {
  return QueryableUtility::FindIntersect(relationship_manager_.GetNext(statement_number),
                       entity_queryable_.QueryAll(type));
}
...
std::unordered_set<std::string> CfgQueryable::QueryPrevious(
  ...
  return QueryableUtility::FindIntersect(relationship_manager_.GetPrevious(statement_number),
                       entity_queryable_.QueryAll(type));
}
...
std::unordered_set<std::string> CfgQueryable::QueryNextT(int statement_number,
                                                         EntityType type) const {
  return QueryableUtility::FindIntersect(relationship_manager_.GetNextT(statement_number),
                       entity_queryable_.QueryAll(type));
}
...
std::unordered_set<std::string> CfgQueryable::QueryPreviousT(
    int statement_number, EntityType type) const {
  return QueryableUtility::FindIntersect(relationship_manager_.GetPreviousT(statement_number),
                       entity_queryable_.QueryAll(type));
}
