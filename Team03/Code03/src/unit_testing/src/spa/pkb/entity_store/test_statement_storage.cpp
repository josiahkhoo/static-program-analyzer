...
TEST_CASE("Call Storage Management", "[StatementStorage]") {
  TNode mock_call_node = TNode(1, TNode::Call, 1, "call_proc");
...
TEST_CASE("Read Storage Management", "[StatementStorage]") {
  TNode mock_variable_node = TNode(1, TNode::Variable, 1, "a");
  std::vector<std::shared_ptr<TNode>> children = {
      std::make_shared<TNode>(mock_variable_node)};
  TNode mock_read_node = TNode(2, TNode::Read, 2, children);
...
TEST_CASE("Print Storage Management", "[StatementStorage]") {
  ...
  std::vector<std::shared_ptr<TNode>> children = {
      std::make_shared<TNode>(mock_variable_node)};
  TNode mock_print_node = TNode(3, TNode::Print, 3, children);
