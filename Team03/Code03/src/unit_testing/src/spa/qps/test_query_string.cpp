...
TEST_CASE("Test with 'Assign a; Select a such that Follows(1, a)'",
  ...
  REQUIRE_FALSE(std::dynamic_pointer_cast<Clause>(qs.GetQueryOperation()[0])->GetLeftHandSide().IsSynonym());
  REQUIRE(std::dynamic_pointer_cast<Clause>(qs.GetQueryOperation()[0])->GetRightHandSide().GetSynonym() ==
